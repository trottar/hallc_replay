...
void KaonYield::Terminate()
{
  ...
  cout << Form("Number of good kaon events: %.0f +/- %.0f\n",h1mmiss_cut->GetEntries(),sqrt(h1mmiss_cut->GetEntries()));

  ofstream output; output.open("output.txt", ios::out | ios::trunc);
  output << h1mmiss_cut->GetEntries();
  output << " ";
  output << sqrt(h1mmiss_cut->GetEntries());
  output.close();
}
